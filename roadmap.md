...
- [x] שחקנים: CRUD + סטטוסים (פעיל/לא פעיל/עזב) + שיבוץ לקבוצה בעונה
- [x] מאמנים: CRUD (פרטים, הסמכות, תוקף רישיון) + שיוך עונתי לקבוצה
- [x] גלגול עונה (Rollover): העתקת מבנה עונה קודמת *(קידום לפי גיל — שדרוג עתידי)* + read-only לעונה סגורה
- [x] Branding: לוגו (Supabase Storage, PNG/JPG) + צבעים + שם תצוגה נטענים בזמן ריצה (CSS vars מ-color-mix) + ערכת נושא בהיר/כהה/מערכת + loader החלה
- [x] דשבורד מועדון: שחקנים בעונה, מאמנים, שיבוצים *(התפלגות/פעילות — שדרוג עתידי)*
- [x] **CMS UI/UX (סוף Phase 1):** מעטפת CMS — **סייד-בר ניווט** (לוגו/מועדון, אייקונים, עונה פעילה), **טבלאות נתונים מקצועיות** לכל הרשימות, **ערכת נושא בהיר/כהה/מערכת**, מצבי ריק, עקביות עם design-system.
...
### Payments Flow — הרשמה עצמית ציבורית + תשלום (feature קריטי, branch `payments-flow`)

> דף ציבורי (בלי התחברות) להרשמה עצמית: הורה/בוגר ממלא פרטים ומשלם; **רק אם התשלום מאושר** נוצרים שחקן + איש קשר + קישור + חיוב-משולם. קישור קבוע פר-מועדון: `app.domain/register/<slug>`.

- [ ] סכמה: `registrations` (טיוטת רישום ממתינה) + `registration_fee_agorot` ב-`billing_settings` + RLS + בדיקת בידוד
- [ ] הגדרת דמי רישום במסך ההגדרות
- [ ] שכבת ספק תשלום מופשטת + מימוש **mock** (סימולציית הצלחה לבדיקות)
- [ ] דף `/register/[slug]` — טופס דו-שלבי: (1) פרטי הורה+ילד / בוגר-עצמאי (2) תשלום. זיהוי מועדון מה-slug (service role)
- [ ] מימוש (materialization) בהצלחת תשלום: find-or-create שחקן לפי ת.ז. → איש קשר (`self`/`parent`, `is_billing`) → קישור → חיוב משולם + תשלום → סימון הרישום כהושלם (אטומי)
- [ ] webhook אמיתי (אימות חתימה + idempotency) — כשיחובר Tranzila
- [ ] טבלת שחקנים: אינדיקטור "שילם / חוב" (= אין יתרה פתוחה)
...
## Phase 6 — הרחבות (התראות, מוצרים, נוכחות)

**מערכת התראות/הודעות (Notifications)**
> שכבה אחת שכמה פיצ'רים רוכבים עליה (תפוצה, פיגור בתשלום, היעדרות).
- [ ] שכבת notifications מופשטת (ספק מוחלף — SMS/WhatsApp דרך Twilio)
- [ ] **opt-in** — checkbox בהרשמה + שדה הסכמה על איש הקשר (חובה לאסוף מוקדם)
- [ ] click-to-chat בוואטסאפ לאיש קשר בודד (`wa.me`, מיידי, בלי API)
- [ ] תפוצת **SMS לכולם** (Twilio) — v1 של תפוצה (ללא אישור תבניות)
- [ ] תפוצת **WhatsApp Business API** + תבניות מאושרות (Meta) + opt-in — v2
- [ ] מנוע התראות אוטומטיות (פיגור בתשלום, היעדרות) על השכבה

**תשלום לפי מוצר (קטלוג)**
- [ ] הרחבת `payment_plans` לקטלוג מוצרים: `type` (רישום/ביגוד/שנתי/תרומה/ספר) + סכום (תרומה = חופשי)
- [ ] מסך ניהול מוצרים
- [ ] חיוב לפי מוצר · add-ons בהרשמה הציבורית · תרומה בסכום חופשי

**נוכחות ומאמנים**
- [ ] סיכום נוכחות פר-שחקן (הגיע/סה"כ) לעונה, במסך המאמן
- [ ] התראת היעדרות — שחקן שהחסיר >2 אימונים מסומן (⚠️) + מתחבר למנוע ההתראות
