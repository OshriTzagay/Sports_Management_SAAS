...
- **RBAC פעיל ונאכף.** כל action רגיש עטוף ב-`requirePermission(<key>)` (default-deny), ה-UI מסונן לפי הרשאה, ומסך צוות מאפשר ל-Owner להזמין/לשנות role/להשבית/להסיר.
- **התחברות:** אימייל+סיסמה ו-SMS OTP (שניהם על אותו חשבון). איפוס סיסמה עם SMS. השבתה חוסמת כניסה עם הודעה.
- **אימונים (trainings):** מומש במלואו — `trainings.view/manage`, מסך מאמן (יצירה/התחלה/נוכחות/סיום), נעילה אחרי סיום ל-Owner בלבד, וסיכום למנהל (אימונים + שעות) בכרטיס המאמן. קישור משתמש↔כרטיס מאמן דרך `person_id`.
- **קטלוג הרשאות עדכני:** 18 — כולל `trainings.view/manage`. Owner=18 · גזבר/ית=10 · מאמן=7.
