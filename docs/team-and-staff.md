...
- **self-service לטלפון** (משתמש מעדכן טלפון משלו עם אימות OTP; כרגע Owner מעדכן).
- audit log גלוי לפעולות צוות/הרשאות.

## נעשה לאחרונה
- כפיית החלפת סיסמה בכניסה ראשונה (`must_change_password` → `/set-password`).
- Owner מגדיר/מעדכן טלפון למשתמש (כולל לעצמו) לכניסה ב-SMS.
